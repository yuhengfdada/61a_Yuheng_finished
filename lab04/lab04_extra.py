...
def put_piece(board, max_rows, column, player):
    ...
    if index_row != -1:
        new_board[index_row] = replace_elem(new_board[index_row],column,player)
    return (index_row,new_board)
...
def print_board(board, max_rows, max_cols):
    ...
    for row in range(max_rows):
        ...
        for col in range(max_cols):
            str_row = str_row + get_piece(board,row,col) + ' '
        print(str_row[:-1])
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    adjacent = 0
    col_left = 0
    while col_left < max_cols:
        piece = get_piece(board, row, col_left)
        if piece == player:
            adjacent += 1
        else:
            adjacent = 0
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    adjacent = 0
    row_top = 0
    while row_top < max_rows:
        piece = get_piece(board, row_top, col)
        if piece == player:
            adjacent += 1
        else:
            adjacent = 0
