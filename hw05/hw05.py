...
def is_weight(w):
    ...
    return not is_mobile(w) and type(label(w)) == int and not branches(w)
...
def balanced(m):
    ...
    side_1 = sides(m)[0]
    side_2 = sides(m)[1]
    if total_weight(end(side_1))*length(side_1) != total_weight(end(side_2))*length(side_2):
        return False
    return balanced(end(side_1)) and balanced(end(side_2))
...
def make_fib():
    ...
    def fib():
        ...
        if init2:
            init2 = False
            return 1
        res,prev1,prev2 = prev1+prev2,prev2,prev1+prev2
        return res
    return fib
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount,input_pwd):
        ...
        if input_pwd != password:
            wrong_attempts.append(input_pwd)
            num_wrongs += 1
...
def make_joint(withdraw, old_password, new_password):
    ...
    if type(res) == str:
        return res
    def joint(amount,input_pwd):
        if input_pwd != new_password:
            return withdraw(amount,input_pwd)
        else:
...
def check_par():
    ...
    True
    """
    r1 = interval(-0.9,-0.1) # Replace this line!
